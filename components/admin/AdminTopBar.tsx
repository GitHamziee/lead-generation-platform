...
export default function AdminTopBar() {
  ...
  return (
    <header className="sticky top-0 z-20 flex h-16 items-center gap-4 border-b border-slate-200 dark:border-slate-700 bg-white/95 dark:bg-slate-900/95 backdrop-blur-sm px-4 sm:px-6">
      ...
      {/* Page title */}
      <h1 className="text-sm sm:text-lg font-semibold text-slate-900 dark:text-white">{pageTitle}</h1>
      ...
      <div className="flex items-center gap-3">
        <ThemeToggle />
        <span className="text-sm font-medium text-slate-700 dark:text-slate-300 hidden sm:block">{userName}</span>
